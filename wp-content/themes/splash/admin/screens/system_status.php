...
<div class="wrap about-wrap stm-admin-wrap stm-system-status stm-admin-status-screen">
	...
	<h3 class="screen-reader-text"><?php _e('WordPress Environment', 'splash'); ?></h3>
	<table class="widefat" cellspacing="0">
		<thead>
		<tr>
			<th colspan="3"
			    data-export-label="WordPress Environment"><?php _e('WordPress Environment', 'splash'); ?></th>
		</tr>
		</thead>
		<tbody>
		<tr>
			<td data-export-label="Home URL"><?php _e('Home URL:', 'splash'); ?></td>
			<td class="help"><?php echo '<a href="#" class="help_tip" data-tip="' . esc_attr__('The URL of your site\'s homepage.', 'splash') . '">[?]</a>'; ?></td>
			<td><?php echo home_url(); ?></td>
		</tr>
		<tr>
			<td data-export-label="Site URL"><?php _e('Site URL:', 'splash'); ?></td>
			<td class="help"><?php echo '<a href="#" class="help_tip" data-tip="' . esc_attr__('The root URL of your site.', 'splash') . '">[?]</a>'; ?></td>
			<td><?php echo site_url(); ?></td>
		</tr>
		<tr>
			<td data-export-label="WP Version"><?php _e('WP Version:', 'splash'); ?></td>
		...
		<tr>
			<td data-export-label="WP Multisite"><?php _e('WP Multisite:', 'splash'); ?></td>
		...
		<tr>
			<td data-export-label="WP Memory Limit"><?php _e('WP Memory Limit:', 'splash'); ?></td>
			...
			<td>
				...
				if ($memory < 128000000) {
					...
					if ($memory < 256000000) {
						echo '<br /><mark class="error">' . __('Your current memory limit is sufficient, but if you need to import classic demo content, the required memory limit is <strong>256MB.</strong>', 'splash') . '</mark>';
					}
				}
				?>
			</td>
		...
		<tr>
			<td data-export-label="WP Debug Mode"><?php _e('WP Debug Mode:', 'splash'); ?></td>
		...
		<tr>
			<td data-export-label="Language"><?php _e('Language:', 'splash'); ?></td>
	...
	<h3 class="screen-reader-text"><?php _e('Server Environment', 'splash'); ?></h3>
	<table class="widefat" cellspacing="0">
		<thead>
		<tr>
			<th colspan="3" data-export-label="Server Environment"><?php _e('Server Environment', 'splash'); ?></th>
		</tr>
		</thead>
		<tbody>
		<tr>
			<td data-export-label="Server Info"><?php _e('Server Info:', 'splash'); ?></td>
			<td class="help"><?php echo '<a href="#" class="help_tip" data-tip="' . esc_attr__('Information about the web server that is currently hosting your site.', 'splash') . '">[?]</a>'; ?></td>
			<td><?php echo esc_html($_SERVER['SERVER_SOFTWARE']); ?></td>
		</tr>
		<tr>
			<td data-export-label="PHP Version"><?php _e('PHP Version:', 'splash'); ?></td>
		...
		<?php if (function_exists('ini_get')) : ?>
			<tr>
				<td data-export-label="PHP Post Max Size"><?php _e('PHP Post Max Size:', 'splash'); ?></td>
			...
			<tr>
				<td data-export-label="PHP Time Limit"><?php _e('PHP Time Limit:', 'splash'); ?></td>
				...
				<td>
					...
					if (180 > $time_limit && 0 != $time_limit) {
						...
						if (300 > $time_limit && 0 != $time_limit) {
							echo '<br /><mark class="error">' . __('Current time limit is sufficient, but if you need import demo content, the required time is <strong>300</strong>.', 'splash') . '</mark>';
						}
					}
					?>
				</td>
			...
			<tr>
				<td data-export-label="PHP Max Input Vars"><?php _e('PHP Max Input Vars:', 'splash'); ?></td>
			...
			<tr>
				<td data-export-label="SUHOSIN Installed"><?php _e('SUHOSIN Installed:', 'splash'); ?></td>
			...
			<?php if (extension_loaded('suhosin')) : ?>
				<tr>
					<td data-export-label="Suhosin Post Max Vars"><?php _e('Suhosin Post Max Vars:', 'splash'); ?></td>
					<td class="help"><?php echo '<a href="#" class="help_tip" data-tip="' . esc_attr__('The maximum number of variables your server can use for a single function to avoid overloads.', 'splash') . '">[?]</a>'; ?></td>
					<?php
					$registered_navs = get_nav_menu_locations();
					$menu_items_count = array('0' => '0');
				...
				<tr>
					<td data-export-label="Suhosin Request Max Vars"><?php _e('Suhosin Request Max Vars:', 'splash'); ?></td>
					<td class="help"><?php echo '<a href="#" class="help_tip" data-tip="' . esc_attr__('The maximum number of variables your server can use for a single function to avoid overloads.', 'splash') . '">[?]</a>'; ?></td>
					<?php
					$registered_navs = get_nav_menu_locations();
					$menu_items_count = array('0' => '0');
				...
				<tr>
					<td data-export-label="Suhosin Post Max Value Length"><?php _e('Suhosin Post Max Value Length:', 'splash'); ?></td>
		...
		<?php endif; ?>
		<tr>
			<td data-export-label="ZipArchive"><?php _e('ZipArchive:', 'splash'); ?></td>
		...
		<tr>
			<td data-export-label="MySQL Version"><?php _e('MySQL Version:', 'splash'); ?></td>
		...
		<tr>
			<td data-export-label="Max Upload Size"><?php _e('Max Upload Size:', 'splash'); ?></td>
	...
	<h3 class="screen-reader-text"><?php _e('Active Plugins', 'splash'); ?></h3>
	<table class="widefat" cellspacing="0" id="status">
		<thead>
		<tr>
			<th colspan="3"
			    data-export-label="Active Plugins (<?php echo count((array)get_option('active_plugins')); ?>)"><?php _e('Active Plugins', 'splash'); ?>
		...
		foreach ($active_plugins as $plugin) {
			...
			if (!empty($plugin_data['Name'])) {
				...
				if (!empty($plugin_data['PluginURI'])) {
					$plugin_name = '<a target="_blank" href="' . esc_url($plugin_data['PluginURI']) . '" title="' . __('Visit plugin homepage', 'splash') . '">' . $plugin_name . '</a>';
				}
				?>
				<tr>
					<td><?php echo $plugin_name; ?></td>
					<td class="help">&nbsp;</td>
					<td><?php printf(_x('by %s', 'by author', 'splash'), $plugin_data['Author']) . ' &ndash; ' . esc_html($plugin_data['Version']) . $version_string . $network_string; ?></td>
				</tr>
				<?php
			}
		}
