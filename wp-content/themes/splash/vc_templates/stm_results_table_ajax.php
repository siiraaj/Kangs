...
<div class="stm-events-wrap <?php echo esc_attr($css_class) ?>">
    <div class="stm-single-events-title_box clearfix">
        <h4 class="sp-table-caption"><?php echo firstWordBold($title); ?><a href="?sp_calendar=fixtures-list" class="stm-link-all stm-link-all-<?php echo esc_attr($results_type); ?> normal-font"><?php echo esc_html__('Full table', 'splash'); ?></a></h4>
    </div>
            ...
            <select name="events_league_list_<?php echo esc_attr($results_type); ?>">
                <?php
                    foreach ($taxList as $k => $val) {
                        ?>
                        <option value="<?php echo $val->term_id; ?>"><?php echo esc_html($val->name); ?></option>
                        <?php
                    }
                ?>
            </select>
