...
?>
	<div class="stm-media-tabs stm-news-tabs-wrapper <?php echo $news_tabs_style?>">
		<div class="clearfix">
			<?php if ( ! empty( $title ) ): ?>
					...
					<?php foreach($post_categories as $post_category): $counter++; ?>
						...
						if(!empty($category)): ?>
							<li <?php if($counter == 1): ?>class="active"<?php endif; ?>>
								<a href="#<?php echo esc_attr($category->slug) ?>" aria-controls="<?php echo esc_attr($category->slug) ?>" role="tab" data-toggle="tab" <?php if(is_layout('baseball')) :?>class="normal_font"<?php endif; ?>>
					...
					if($post_query->have_posts()): ?>
                        ...
                        <?php if($stm_show_load_more == "enable" ): ?>
                            <div class="stm-load-more-btn">
                                <a href="<?php echo esc_url(get_term_link($post_category, 'category')); ?>" class="button only_border">
