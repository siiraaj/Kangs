...
<div class="stm_latest_news_most_styles <?php echo esc_attr($css_class . ' ' . $view_style); ?>">
	...
	<?php if(!empty($title)): ?>
		<?php if(is_layout('soccer_two')): ?>
	        <h2><?php echo firstWordBold($title); ?></h2>
		<?php else: ?>
			<h4><?php echo firstWordBold($title); ?></h4>
		<?php endif; ?>
	<?php endif; ?>
		    ...
		    wp_reset_postdata();
		        ...
		        <div class="stm-post-masonry-wrap row">
			        <?php if($news_page_url): ?>
		                <a href="<?php echo esc_url($news_page_url); ?>" class="news-page-link button btn-only-border">
			                <?php echo esc_html_e('All News'); ?>
			        ...
			        while ($query->have_posts()):
						...
						<a href="<?php the_permalink(); ?>" class="stm-masonry-post-url">
							<div class="stm-post-masonry-wrap__item <?php echo $count == 1 ? 'col-md-8 col-sm-8 big-post' : 'col-md-4 col-sm-4 col-xs-6 small-post'; ?>">
								<?php
    ...
    <div class="load-more-btn-wrap">
        <a href="#" class="load-posts-more-style load-more-btn heading-font" <?php echo $data; ?>>
            <?php echo esc_html__($load_mpre_btn_title, 'splash');?>
        </a>
    </div>
    <?php endif; ?>
</div>
