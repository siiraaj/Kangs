...
$bgImg = (!empty($use_background_image) && $use_background_image == 'enable' && !empty($background_image)) ? 'style="background-image: url(' . wp_get_attachment_image_url($background_image, 'full') . '); background-size: cover;"' : 'style="background-image: url(' . get_the_post_thumbnail_url($player_id, 'full') . '); background-size: cover;"';
?>
<div class="stm_player_month" <?php echo $bgImg; ?>>
    <a href="<?php echo get_the_permalink($player_id); ?>" >
        <div class="player-info">
            <div class="title normal_font"><?php echo esc_html($title);?></div>
            <div class="player-name heading-font"><?php echo firstWordBold($player->post_title); ?></div>
