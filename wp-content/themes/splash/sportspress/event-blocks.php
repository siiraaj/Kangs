                ...
                if (count($teams) > 1):
						...
						if ( has_post_thumbnail ( $team ) ):
							...
							if ( $link_teams ):
								if(is_layout("bb")) {
									...
									if(!is_layout('baseball') && !is_layout('magazine_one')) {
					...
					<div class="stm-single-block-event-list sp-stm-template-event-blocks-<?php echo esc_attr(strtolower($event->post_status)); ?>">
						<?php if(!is_layout("sccr") && !is_layout('soccer_two')) : ?>
							<?php if(is_layout("bb") || is_layout("magazine_one")) : ?><a href="<?php echo esc_url(get_the_permalink($event->ID)); ?>" class="stm-no-decor"><?php endif; ?>
								<?php if(is_layout("bb") || is_layout("magazine_one")): ?><!--is not americanfootball-->
									<div class="stm-single-block-event-list-top">
										...
										<?php
										if(is_layout("bb") || is_layout("magazine_one")):
											if ( $show_venue ):
								...
								<div class="stm-single-block-unit">
									...
									<div class="stm-teams-info heading-font">
										...
										<div class="stm-title-team">
											<?php if(is_layout("af") || is_layout("baseball")):?>
												<span class="stm-team-city"><?php echo $city_1[0]->name;?></span>
												<span class="stm-team-name"><?php echo esc_attr(get_the_title($team_1_id)); ?></span>
												<?php if(is_layout('baseball') && !empty($team_results[0]) && !empty($team_results[0][$team_1_id]) && isset($team_results[0][$team_1_id]['outcome']) and !empty($team_results[0][$team_1_id]['outcome'][0])): ?>
                                                    <div class="<?php echo ($team_results[0][$team_1_id]['outcome'][0] == 'win') ? 'stm-latest-result-win-label' : 'stm-latest-result-lose-label';?> heading-font"><?php esc_html_e($team_results[0][$team_1_id]['outcome'][0], 'splash') ?></div>
												<?php endif; ?>
											<?php else: ?>
										...
										<div class="stm-title-team opponent">
											<?php if(is_layout("af") || is_layout("baseball")): ?>
												<span class="stm-team-city"><?php echo $city_2[0]->name;?></span>
												<span class="stm-team-name"><?php echo esc_attr(get_the_title($team_2_id)); ?></span>
												<?php if(is_layout('baseball') && !empty($team_results[0]) && !empty($team_results[0][$team_2_id]) && isset($team_results[0][$team_2_id]['outcome']) and !empty($team_results[0][$team_2_id]['outcome'][0])): ?>
                                                    <div class="<?php echo ($team_results[0][$team_2_id]['outcome'][0] == 'win') ? 'stm-latest-result-win-label' : 'stm-latest-result-lose-label';?> heading-font"><?php esc_html_e($team_results[0][$team_2_id]['outcome'][0], 'splash') ?></div>
                                                <?php endif; ?>
		                                    <?php else: ?>
								...
								<?php if(is_layout("af") || is_layout("baseball")): ?><!--is americanfootball-->
									<div class="stm-single-block-event-list-top">
										...
										<div class="stm-link-wrapp">
											<?php
                                            $btnClass = (is_layout('baseball')) ? 'bsbl-btn' : 'button';


											echo '<a class="' . $btnClass . '" href="' . get_post_permalink( $event->ID, false, true ) . '">';
											if(!is_layout("baseball")) echo '<i class="fa stm-icon-ico_camera" aria-hidden="true"></i>';
								...
								<?php endif; ?>
							<?php if(is_layout("bb") || is_layout("magazine_one")): ?></a><?php endif; ?>
						<?php elseif(is_layout("sccr") || is_layout('soccer_two')) : ?><!--LAYOUT SOCCER-->
								...
								<div class="fixture_info">
									...
									if ( $show_venue ):
										...
										foreach( $venues as $venue ): ?>
											<div class="venue"><i class="fa fa-location-arrow" aria-hidden="true"></i><?php echo esc_html( $venue->name ); ?></div>
											<?php
										endforeach;
									endif;?>
									<a class="button button-gray btn-only-border" href="<?php echo esc_url( get_the_permalink($event->ID) ); ?>">
