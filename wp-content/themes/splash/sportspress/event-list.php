...
$bsbNormalClass = (is_layout('baseball')) ? "normal_font" : "";


?>
<?php if($event_list_template == 'theme') { ?>
	<div class="stm-upcoming-events_list <?php echo $paginated ? 'paginated-list' : ''; ?>" data-rows="<?php echo esc_attr($rows); ?>">
		<?php if(is_layout("bb") || is_layout("magazine_one") || is_layout("magazine_two")) : ?>
		<ul>
	...
	<?php elseif(is_layout("af") || is_layout('baseball')): /*==========================================================        AMERICAN FOOTBALL or BASEBALL  ========================================================= */ ?>
					...
					<table class="sp-event-list sp-data-table<?php if ( $paginated ) { ?> sp-paginated-table<?php } if ( $sortable ) { ?> sp-sortable-table<?php } if ( $scrollable ) { ?> sp-scrollable-table<?php } ?>" data-sp-rows="<?php echo esc_attr($rows); ?>">
						...
						foreach ( $data as $event ):
							...
							if ( sp_column_active( $usecolumns, 'article' ) ):
								...
								$classArticle = (is_layout('baseball')) ? 'normal_font' : 'heading-font';
	...
	<?php
        elseif(is_layout("sccr") || is_layout('soccer_two')): /*=======================   SOCCER   ======================== */ ?>
						...
						<li class="clearfix">
							...
							<div class="clearfix">
								<div class="stm-event-data-wrap">
									<div class="commands <?php echo !$result ? 'no-result' : ''; ?>">
										<?php if($teams){ ?>
											<h3>
												...
												<span class="event-vs-mobile" style="display: none;"><?php echo ($result != "") ? wp_kses_post( $result ) : "vs"; ?></span>
										...
										<div class="stm-event-bottom-info">
											<div class="date-time">
												...
												<span class="mobile-show" style="display: none;"><?php echo get_post_time( 'j F, h:i A', false, $event, true ); ?></span>
											</div>
											<?php
											$venues = get_the_terms( $event->ID, 'sp_venue' );
											if ( $venues ){
									...
									<?php endif; ?>
									<div class="read_more">
										<a class="button button-gray btn-only-border" href="<?php echo esc_url( get_post_permalink( $event->ID, false, true ) ); ?>">
...
<?php } else { ?>
	<div class="sp-template sp-template-event-list">
		...
		<?php } ?>
		<div class="sp-table-wrapper">
			<table class="sp-event-list sp-event-list-format-<?php echo $title_format; ?> sp-data-table<?php if ( $paginated ) { ?> sp-paginated-table<?php } if ( $sortable ) { ?> sp-sortable-table<?php } if ( $scrollable ) { ?> sp-scrollable-table<?php } ?>" data-sp-rows="<?php echo esc_attr($rows); ?>">
				<thead>
					<tr>
						<?php
						echo '<th class="data-date">' . esc_html__( 'Date', 'splash' ) . '</th>';
						...
						if ( sp_column_active( $usecolumns, 'day' ) )
							echo '<th class="data-day">' . __( 'Match Day', 'splash' ) . '</th>';
						?>
					</tr>
				</thead>
				<tbody>
					...
					foreach ( $data as $event ):
						...
						echo '<td class="data-date" itemprop="startDate" content="' . mysql2date( 'Y-m-d\TH:iP', $event->post_date ) . '" data-label="'.__( 'Date', 'sportspress' ).'">' . $date_html . '</td>';
								...
								default:
									if ( sp_column_active( $usecolumns, 'event' ) ) {
										if ( $title_format == 'teams' ) {
											...
											echo '<td class="data-event heading-font" data-label="'.__( 'Event', 'sportspress' ).'">' . $title_html . '</td>';
										}
									}
							...
							if ( sp_column_active( $usecolumns, 'article' ) ):
								...
								echo '<td class="data-article ' . $classArticle . '" data-label="'.__( 'Article', 'sportspress' ).'">';
									if ( $link_events ) echo '<a href="' . get_post_permalink( $event->ID, false, true ) . '">';
