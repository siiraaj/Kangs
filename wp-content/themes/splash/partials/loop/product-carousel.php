...
if ( class_exists( 'WooCommerce' ) ):
	...
	$image = (!is_af()) ? splash_get_thumbnail_url($post_id, 0, 'stm-570-350') : splash_get_thumbnail_url($post_id, 0, 'stm-270-370') ;
	if(is_layout('soccer_two')){
		$image = splash_get_thumbnail_url($post_id, 0, 'post-457-470');
	}
	if(is_layout("baseball")) $image = get_the_post_thumbnail_url( $post->ID, 'shop_thumbnail' );
	...
	<div class="stm-single-product-carousel">
        ...
        if(is_layout("baseball")) {
			if ($product->is_on_sale()) {
				echo apply_filters( 'woocommerce_sale_flash', '<span class="onsale normal_font">' . esc_html__( 'Sale!', 'woocommerce' ) . '</span>', $post, $product );
			}
		}
        ?>
			...
			<?php if(!empty($image)): ?>
				<div class="image">
					...
					<div class="stm-add-to-cart">
						<?php if(is_af()): ?><span class="button btn-secondary btn-style-4"><?php endif; ?>
							<span
								rel="nofollow"
								data-quantity="1"
								data-product_id="<?php echo intval($post_id); ?>"
								data-product_sku=""
								class="<?php if(!is_af()) echo "button btn-secondary btn-style-4"; ?> add_to_cart_button ajax_add_to_cart">
								<?php esc_html_e('Add to cart', 'splash'); ?>
							</span>
						<?php if(is_af()): ?></span><?php endif; ?>
					</div>
                    <?php endif; ?>
				</div>
			<?php endif; ?>
